...
public class Receive {
	...
	private ClientGUIReceive interfaz;
	private PrivateGUIReceive converPrivada;
	...
	public void inicializarGUI() {
		interfaz = new ClientGUIReceive(this);
	}
	...
	public void crearConexion(String nombre) {
		try {
			...
			client = new ChatClientImplReceive(interfaz, this);
	...
	public void conectarClientesPrivado() throws RemoteException {
		...
		converPrivada = new PrivateGUIReceive(this);
	...
	public void enviarPrivado() {
		...
		try {
			...
			List<ConversacionPrivada> cP =  chatServer.getConversacionesPrivadas();
			for(ConversacionPrivada c : cP){
				if(c.getSender().equals(this)){
					c.getReceiver().callback(msg, nombreUsuario);
				}else{
					c.getSender().callback(msg, nombreUsuario);
				}
			}
		} catch (RemoteException e) {
			e.printStackTrace();
	...
	public void conectaMR(ChatClient ch, String nombreUsuario, String nick, String mensaje) {
		if(converPrivada==null){
			...
			try {
				ch.callback("quiere hablar contigo", nombreUsuario);
				ch.callback(mensaje, nick);
			} catch (RemoteException e) {
				e.printStackTrace();
			}
		}
